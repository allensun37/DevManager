...
- [ ] 首次推送前，在 GitHub 页面确认提交记录显示为新账号。

## 架构约束

- [ ] 菜单层只处理循环菜单、输入和输出。
- [ ] `ProjectManager` 只处理业务规则、项目集合和 ID 分配。
- [ ] `Project` 只表示项目数据及其 JSON 转换。
- [ ] JSON 文件读写封装在仓储层，不写入菜单层或 `ProjectManager`。
- [ ] 业务层仅依赖仓储接口，不能依赖 JSON 细节。
...
- [ ] 定义 `Project`：ID、名称、技术栈标签、描述、自由文本状态。
- [ ] 明确字段规则：名称和状态非空；技术栈至少一个非空标签；描述可空。
- [ ] 设计 JSON 中项目对象的固定字段名。
- [ ] 为字段校验和 JSON 往返转换准备测试用例。
...
- [ ] 自动生成并维护不复用的永久 ID。
- [ ] 实现名称和技术栈的忽略大小写部分匹配。
- [ ] 将 `C++` 与 `cpp` 的规范化规则集中在一个位置。
- [ ] 覆盖空输入、无效 ID、删除后 ID 不复用等边界测试。
- [ ] 提交：`feat: implement project management operations`

### 阶段 4：JSON 持久化

- [ ] 定义仓储接口与 JSON 实现。
- [ ] 在 `data/projects.json` 保存 `nextId` 和 `projects`。
- [ ] 文件缺失时加载空库。
- [ ] JSON 损坏时返回明确错误，且不覆盖原文件。
- [ ] 验证重启后的项目数据与 `nextId` 能正确恢复。
- [ ] 提交：`feat: add JSON project persistence`

### 阶段 5：命令行交互

- [ ] 实现循环菜单和退出流程。
- [ ] 对数字输入、空行、错误 ID 提供可重试的提示。
- [ ] 删除前要求 y/n 确认。
- [ ] 列表结果明确显示真实项目 ID。
- [ ] 验证成功和失败路径都不会导致程序异常退出。
- [ ] 提交：`feat: add interactive command-line menu`
...
- [ ] 新增项目后重启，数据仍存在。
- [ ] 删除一个项目后，新项目得到新的 ID 而非被删除的 ID。
- [ ] `server` 能匹配 `HTTP Server`。
- [ ] `cpp` 能匹配标签 `C++`。
- [ ] 项目名称或状态为空时不能新增。
- [ ] 技术栈全为空标签时不能新增。
- [ ] `projects.json` 缺失时可正常启动。
- [ ] `projects.json` 损坏时程序提示错误且不覆盖该文件。
